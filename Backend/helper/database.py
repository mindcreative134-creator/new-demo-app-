...
from Backend.helper.modal import (
    Episode, MovieSchema, QualityDetail, Season, TVShowSchema,
    ChannelSchema, EditorialPostSchema, SportsFixtureSchema
)


class Database:
    def __init__(self, connection_uri: Union[str, List[str]] = Telegram.DATABASE, db_name: str = "projectS"):
        self._conn = None
        self.db = None
    ...
    async def connect(self):
        ...
        try:
            ...
            # Ensure we extract a single string URI even if connection_uri is a list
            uri = self.connection_uri
            if isinstance(uri, list):
                uri = uri[0] if uri else ""

            LOGGER.info(f"Connecting to database URI: {uri}")
    ...
    async def insert_media(
        ...
        if metadata_info['media_type'] == "movie":
            try:
                ...
                LOGGER.error(f"Pydantic validation failed for Movie: {e}. Falling back to direct database raw write.")
                movie_dict = {
                ...
                existing_media = await self.movie_collection.find_one({"tmdb_id": movie_dict["tmdb_id"]})
                if not existing_media:
                    result = await self.movie_collection.insert_one(movie_dict)
                    return result.inserted_id
                else:
                    ...
                    if updated:
                        ...
                        existing_media["languages"] = movie_dict["languages"]
                        existing_media["rip"] = movie_dict["rip"]
                        await self.movie_collection.replace_one({"tmdb_id": movie_dict["tmdb_id"]}, existing_media)
                    return existing_media["_id"]
        else:
            try:
                ...
                LOGGER.error(f"Pydantic validation failed for TV Show: {e}. Falling back to direct database raw write.")
                tv_dict = {
                    "tmdb_id": int(metadata_info.get('tmdb_id', 99999999)),
                    "title": metadata_info.get('title', name),
                    "genres": metadata_info.get('genres', ["Action"]),
                ...
                existing_media = await self.tv_collection.find_one({"tmdb_id": tv_dict["tmdb_id"]})
                if not existing_media:
                    result = await self.tv_collection.insert_one(tv_dict)
                    return result.inserted_id
                else:
                    updated = False
                    ...
                    if updated:
                        ...
                        await self.tv_collection.replace_one({"tmdb_id": tv_dict["tmdb_id"]}, existing_media)
                    return existing_media["_id"]
    ...
    async def delete_document(
        ...
        if media_type == "mov" or media_type == "movie":
    ...
    async def get_channels(self, category: Optional[str] = None) -> List[dict]:
        try:
            query = {"is_active": True}
            if category:
                query["category"] = {"$regex": f"^{category}$", "$options": "i"}
            cursor = self.channels_collection.find(query)
            channels = await cursor.to_list(length=100)
            return [self._convert_object_id(c) for c in channels]
        except Exception as e:
    ...
    async def get_editorial(self, category: Optional[str] = None) -> List[dict]:
        try:
            query = {"is_published": True}
            if category:
                query["category"] = {"$regex": f"^{category}$", "$options": "i"}
            cursor = self.editorial_collection.find(query).sort("published_on", -1)
            posts = await cursor.to_list(length=100)
            return [self._convert_object_id(p) for p in posts]
        except Exception as e:
